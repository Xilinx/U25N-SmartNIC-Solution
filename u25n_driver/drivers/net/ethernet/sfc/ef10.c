...
static bool efx_u25_check_cap(struct efx_ef10_nic_data *nic_data)
{
	...
	/* Checks for the eMCDI capability in the device. */
	rc = check_emcdi_cap(efx);
	if (rc) {
		netif_err(efx, hw, efx->net_dev, "eMCDI capabilities are absent\n");
		u25 = false;
	} else {
		netif_info(efx, drv, efx->net_dev, "Device has eMCDI capabilities\n");
		netif_info(efx, drv, efx->net_dev, "Device is U25\n");
		u25 = true;
	}

	/* If eMCDI is present, we check for FPGA version and external MAC and MAE
	 * support by the FPGA design. Also checking which flash the FPGA is 
	 * currently booting from.
	 */
	if (u25) {
		rc = get_fpga_caps(efx, &fpga);
		if (rc != 0)
			netif_err(efx, drv, efx->net_dev, "Error in obtaining FPGA capabilities\n");
		else {
			netif_info(efx, drv, efx->net_dev, "FPGA design %s MACs for network ports\n",
					fpga.mac ? "supports" : "does not support");
			netif_info(efx, drv, efx->net_dev, "FPGA design %s external MAE\n", 
					fpga.mae ? "supports" : "does not support");
		}

		memset(version, 0, sizeof(version));
		rc = get_fpga_vers(efx, version);
		if (rc != 0)
			netif_err(efx, drv, efx->net_dev, "Error in obtaining FPGA version\n");
		else {
			netif_info(efx, drv, efx->net_dev, "FPGA Version: 0x%.2x%.2x%.2x%.2x\n", 
									version[3],
									version[2],
									version[1],
									version[0]);
		}
		if (get_fpga_nvram(efx, &fpga) != 0) {
			netif_err(efx, drv, efx->net_dev, "Unable to get FPGA flash information\n");
		} else
			netif_info(efx, drv, efx->net_dev, "FPGA is booting from %s flash\n", 
					fpga.nvram_id ? "secondary" : "primary");
	}
	return u25;
}
...
static void _efx_ef10_rx_write(struct efx_rx_queue *rx_queue)
{
	...
	while (notified_count != write_count) {
		efx_ef10_build_rx_desc(
			rx_queue,
			notified_count & rx_queue->ptr_mask);
		++notified_count;
	}
...
#if defined(EFX_USE_KCOMPAT) && defined(EFX_TC_OFFLOAD) \
    && !defined(EFX_HAVE_FLOW_INDR_BLOCK_CB_REGISTER) && !defined(EFX_HAVE_FLOW_INDR_DEV_REGISTER)
static struct efx_udp_tunnel2 *__efx_ef10_udp_tnl_find_port2(
		struct efx_ef10_nic_data *nic_data, __be16 port)
__must_hold(nic_data->udp_tunnels_lock)
{
	struct efx_udp_tunnel2 *tnl;

	list_for_each_entry(tnl, &nic_data->udp_tunnel_list, list)
		if (port == tnl->port)
			return tnl;
	return NULL;
}

static void efx_ef10_udp_tnl_add_port2(struct efx_nic *efx,
		struct efx_udp_tunnel2 tnl)
{
	struct efx_ef10_nic_data *nic_data = efx->nic_data;
	struct efx_udp_tunnel2 *entry;

	
	spin_lock(&nic_data->udp_tunnels_lock);
	entry = __efx_ef10_udp_tnl_find_port2(nic_data, tnl.port);
	if (entry) /* EEXIST */
		goto out;
	...
	if (!entry) /* ENOMEM */
		goto out;
	*entry = tnl;
	list_add_tail(&entry->list, &nic_data->udp_tunnel_list);
out:
	spin_unlock(&nic_data->udp_tunnels_lock);
}

enum efx_encap_type efx_ef10_udp_tnl_lookup_port2(struct efx_nic *efx,
		__be16 port)
{
	struct efx_ef10_nic_data *nic_data = efx->nic_data;
	struct efx_udp_tunnel2 *entry;
	...
	spin_lock(&nic_data->udp_tunnels_lock);
	entry = __efx_ef10_udp_tnl_find_port2(nic_data, port);
	if (entry)
		rc = entry->type;
	else
...
static void efx_ef10_udp_tnl_del_port2(struct efx_nic *efx,
		struct efx_udp_tunnel2 tnl)
{
	struct efx_ef10_nic_data *nic_data = efx->nic_data;
	struct efx_udp_tunnel2 *entry;
...
#if !defined(EFX_USE_KCOMPAT) || defined(EFX_TC_OFFLOAD)
static struct net_device *efx_ef10_get_vf_rep(struct efx_nic *efx, unsigned int vf)
{
#if defined(CONFIG_SFC_SRIOV)
	struct efx_ef10_nic_data *nic_data = efx->nic_data;
...
static void efx_ef10_detach_reps(struct efx_nic *efx)
{
	struct efx_ef10_nic_data *nic_data = efx->nic_data;
...
static void efx_ef10_attach_reps(struct efx_nic *efx)
{
	struct efx_ef10_nic_data *nic_data = efx->nic_data;

	if (!efx_is_u25(efx))
		return;
//	if (nic_data->mode == U25_MODE_SWITCHDEV)
...
static void efx_ef10_link_state_change(struct efx_nic *efx)
{
	...
	if (link_state->up)
		efx_u25_start_reps(efx);
	else
		efx_u25_stop_reps(efx);
}
#else /* EFX_TC_OFFLOAD */
void __efx_ef10_detach_reps(struct efx_nic *efx)
{
...
static int efx_ef10_probe_post_io(struct efx_nic *efx)
{
	...
	/* Identify u25 capable hardware */
	nic_data->is_u25 = efx_u25_check_cap(nic_data);
	if (efx_is_u25(efx)) {
		rc = efx_emcdi_init_channel(efx);
		if (rc)
			return rc;
		rc = efx_u25_probe(efx);
		if (rc)
			return rc;
	}
...
#if defined(EFX_USE_KCOMPAT) && defined(EFX_TC_OFFLOAD) \
    && !defined(EFX_HAVE_FLOW_INDR_BLOCK_CB_REGISTER) && !defined(EFX_HAVE_FLOW_INDR_DEV_REGISTER)
        .udp_tnl_add_port2 = efx_ef10_udp_tnl_add_port2,
        .udp_tnl_lookup_port2 = efx_ef10_udp_tnl_lookup_port2,
        .udp_tnl_del_port2 = efx_ef10_udp_tnl_del_port2,
#endif
#if !defined(EFX_USE_KCOMPAT) || defined(EFX_TC_OFFLOAD)
        .get_vf_rep = efx_ef10_get_vf_rep,
        .detach_reps = efx_ef10_detach_reps,
        .attach_reps = efx_ef10_attach_reps,
        .link_state_change = efx_ef10_link_state_change,
#endif
