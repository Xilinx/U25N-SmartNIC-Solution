...
#include "mcdi_pcol_firewall.h"
#include "ef100_rep.h"
#include "efx_devlink.h"
#include "firewall_err.h"

static int __efx_mcdi_rpc(struct efx_nic *efx, unsigned int cmd,
		const efx_dword_t *inbuf, size_t inlen,
		efx_dword_t *outbuf, size_t outlen,
		size_t *outlen_actual)
{
	if (efx_is_u25(efx))
		return efx_emcdi_rpc(efx, cmd, inbuf, inlen, 
			outbuf, outlen, outlen_actual, EMCDI_TYPE_MAE);

	return efx_mcdi_rpc(efx, cmd, inbuf, inlen, 
...
int efx_mae_allocate_mport(struct efx_nic *efx, u32 *id, u32 *label)
{
	...
	rc = __efx_mcdi_rpc(efx, MC_CMD_MAE_MPORT_ALLOC, inbuf, sizeof(inbuf),
...
int efx_mae_free_mport(struct efx_nic *efx, u32 id)
{
	...
	return __efx_mcdi_rpc(efx, MC_CMD_MAE_MPORT_FREE, inbuf, sizeof(inbuf),
			    NULL, 0, NULL);
}
...
void efx_mae_mport_wire(struct efx_nic *efx, u32 *out)
{
	...
	if (efx_is_u25(efx)) {
		int port_num = PCI_FUNC(efx->pci_dev->devfn);

		EFX_POPULATE_DWORD_3(mport,
				MAE_U25_MPORT_SELECTOR_TYPE, MAE_U25_MPORT_SELECTOR_TYPE_PPORT,
				MAE_U25_MPORT_SELECTOR_MGROUP_ID, port_num,
				MAE_U25_MPORT_SELECTOR_PPORT_ID, port_num);
	} else { 
...
void efx_mae_mport_uplink(struct efx_nic *efx __always_unused, u32 *out)
{
	...
	if (efx_is_u25(efx)) {
		...
		EFX_POPULATE_DWORD_4(mport,
				MAE_U25_MPORT_SELECTOR_TYPE, MAE_U25_MPORT_SELECTOR_TYPE_FUNC,
				MAE_U25_MPORT_SELECTOR_MGROUP_ID, port_num,
				MAE_U25_MPORT_SELECTOR_FUNC_PF_ID, port_num,
				MAE_U25_MPORT_SELECTOR_FUNC_VF_ID, MAE_U25_MPORT_SELECTOR_FUNC_VF_ID_NULL);
	} else {
		EFX_POPULATE_DWORD_3(mport,
...
void efx_mae_mport_vf(struct efx_nic *efx __always_unused, u32 vf_id, u32 *out)
{
	...
	if (efx_is_u25(efx)) {
		...
		EFX_POPULATE_DWORD_4(mport,
				...
				MAE_U25_MPORT_SELECTOR_MGROUP_ID, port_num,
				MAE_U25_MPORT_SELECTOR_FUNC_PF_ID, port_num,
				MAE_U25_MPORT_SELECTOR_FUNC_VF_ID, vf_id);
	} else {
		EFX_POPULATE_DWORD_3(mport,
				MAE_MPORT_SELECTOR_TYPE, MAE_MPORT_SELECTOR_TYPE_FUNC,
				MAE_MPORT_SELECTOR_FUNC_PF_ID, MAE_MPORT_SELECTOR_FUNC_PF_ID_CALLER,
...
/* id is really only 24 bits wide for ef100 and 12 bits wide for u25 */
int efx_mae_lookup_mport(struct efx_nic *efx, u32 selector, u32 *id)
{
	...
	rc = __efx_mcdi_rpc(efx, MC_CMD_MAE_MPORT_LOOKUP, inbuf, sizeof(inbuf),
			  outbuf, sizeof(outbuf), &outlen);
	if (rc)
		return rc;
	if (outlen < sizeof(outbuf))
...
int efx_mae_start_counters(struct efx_nic *efx, struct efx_channel *channel)
{
	...
	MCDI_SET_WORD(inbuf, MAE_COUNTERS_STREAM_START_IN_QID, qid);
	MCDI_SET_WORD(inbuf, MAE_COUNTERS_STREAM_START_IN_PACKET_SIZE,
		      efx->net_dev->mtu);
	rc = __efx_mcdi_rpc(efx, MC_CMD_MAE_COUNTERS_STREAM_START,
...
int efx_mae_stop_counters(struct efx_nic *efx, struct efx_channel *channel)
{
	...
	size_t outlen;
	int rc;
	uint32_t final_generation_count;

	MCDI_SET_WORD(inbuf, MAE_COUNTERS_STREAM_STOP_IN_QID, channel->channel);
	rc = __efx_mcdi_rpc(efx, MC_CMD_MAE_COUNTERS_STREAM_STOP,
			  inbuf, sizeof(inbuf), outbuf, sizeof(outbuf), &outlen);
...
void efx_mae_counters_grant_credits(struct work_struct *work)
{
	...
	MCDI_SET_DWORD(inbuf, MAE_COUNTERS_STREAM_GIVE_CREDITS_IN_NUM_CREDITS,
		       credits);
	if (!__efx_mcdi_rpc(efx, MC_CMD_MAE_COUNTERS_STREAM_GIVE_CREDITS,
...
static int efx_mae_get_basic_caps(struct efx_nic *efx, struct mae_caps *caps)
{
	...
	rc = __efx_mcdi_rpc(efx, MC_CMD_MAE_GET_CAPS, NULL, 0, outbuf,
				   ...
				   u8 *field_support)
{
	...
	rc = __efx_mcdi_rpc(efx, cmd, NULL, 0, outbuf, sizeof(outbuf), &outlen);
	if (rc)
		return rc;
...
int efx_mae_allocate_counter(struct efx_nic *efx, struct efx_tc_counter *cnt)
{
	...
	rc = __efx_mcdi_rpc(efx, MC_CMD_MAE_COUNTER_ALLOC, inbuf, sizeof(inbuf),
			  outbuf, sizeof(outbuf), &outlen);
	if (rc)
		return rc;
...
int efx_mae_free_counter(struct efx_nic *efx, u32 id)
{
	...
	rc = __efx_mcdi_rpc(efx, MC_CMD_MAE_COUNTER_FREE, inbuf, sizeof(inbuf),
			  outbuf, sizeof(outbuf), &outlen);
	if (rc)
		return rc;
	/* pcol says this can't happen, since count is 1 */
...
static int efx_mae_encap_type_to_mae_type(enum efx_encap_type type)
{
	...
	case EFX_ENCAP_TYPE_L2GRE:
		return MAE_MCDI_ENCAP_TYPE_L2GRE;
	default:
		return -EOPNOTSUPP;
	}
}
			      ...
			      struct efx_tc_encap_action *encap)
{
	...
	memcpy(MCDI_PTR(inbuf, MAE_ENCAP_HEADER_ALLOC_IN_HDR_DATA),
	       encap->encap_hdr,
	       encap->encap_hdr_len);
	rc = __efx_mcdi_rpc(efx, MC_CMD_MAE_ENCAP_HEADER_ALLOC, inbuf,
			    ...
			    struct efx_tc_encap_action *encap)
{
	...
	return __efx_mcdi_rpc(efx, MC_CMD_MAE_ENCAP_HEADER_UPDATE, inbuf,
			    inlen, NULL, 0, NULL);
}
			  ...
			  struct efx_tc_encap_action *encap)
{
	...
	rc = __efx_mcdi_rpc(efx, MC_CMD_MAE_ENCAP_HEADER_FREE, inbuf,
...
int efx_mae_alloc_action_set(struct efx_nic *efx, struct efx_tc_action_set *act)
{
	...
	rc = __efx_mcdi_rpc(efx, MC_CMD_MAE_ACTION_SET_ALLOC, inbuf, sizeof(inbuf),
			  outbuf, sizeof(outbuf), &outlen);
	if (rc)
		return rc;
	if (outlen < sizeof(outbuf))
...
int efx_mae_free_action_set(struct efx_nic *efx, struct efx_tc_action_set *act)
{
	...
	rc = __efx_mcdi_rpc(efx, MC_CMD_MAE_ACTION_SET_FREE, inbuf, sizeof(inbuf),
			  outbuf, sizeof(outbuf), &outlen);
	if (rc)
		return rc;
	if (outlen < sizeof(outbuf))
				  ...
				  struct efx_tc_action_set_list *acts)
{
	...
	rc = __efx_mcdi_rpc(efx, MC_CMD_MAE_ACTION_SET_LIST_ALLOC, inbuf, inlen,
			  outbuf, sizeof(outbuf), &outlen);
	if (rc)
		goto out_free;
	if (outlen < sizeof(outbuf)) {
				 ...
				 struct efx_tc_action_set_list *acts)
{
	...
	MCDI_SET_DWORD(inbuf, MAE_ACTION_SET_LIST_FREE_IN_ASL_ID,
		       acts->fw_id);
	rc = __efx_mcdi_rpc(efx, MC_CMD_MAE_ACTION_SET_LIST_FREE, inbuf,
			  sizeof(inbuf), outbuf, sizeof(outbuf), &outlen);
	if (rc)
		return rc;
	if (outlen < sizeof(outbuf))
				 ...
				 struct efx_tc_encap_match *encap)
{
	...
	efx_mae_mport_wire(efx, &ingress_port);
	MCDI_STRUCT_SET_DWORD(match_crit,
		MAE_ENC_FIELD_PAIRS_INGRESS_MPORT_SELECTOR, ingress_port);
	...
	MCDI_STRUCT_SET_WORD_BE(match_crit, MAE_ENC_FIELD_PAIRS_ENC_L4_DPORT_BE_MASK,
				~(__be16)0);
	MCDI_STRUCT_SET_BYTE(match_crit, MAE_ENC_FIELD_PAIRS_ENC_IP_PROTO, encap->ip_proto);
	MCDI_STRUCT_SET_BYTE(match_crit, MAE_ENC_FIELD_PAIRS_ENC_IP_PROTO_MASK, ~0);
	rc = __efx_mcdi_rpc(efx, MC_CMD_MAE_OUTER_RULE_INSERT, inbuf,
			  sizeof(inbuf), outbuf, sizeof(outbuf), &outlen);
	if (rc)
		return rc;
	if (outlen < sizeof(outbuf))
				   ...
				   struct efx_tc_encap_match *encap)
{
	...
	rc = __efx_mcdi_rpc(efx, MC_CMD_MAE_OUTER_RULE_REMOVE, inbuf,
			  sizeof(inbuf), outbuf, sizeof(outbuf), &outlen);
	if (rc)
		return rc;
	if (outlen < sizeof(outbuf))
...
int efx_mae_insert_ct(struct efx_nic *efx, struct efx_tc_ct_entry *conn)
{
	...
	rc = __efx_mcdi_rpc(efx, MC_CMD_MAE_TRACK_CONNECTION, inbuf, sizeof(inbuf),
			  outbuf, sizeof(outbuf), &outlen);
	if (rc)
		return rc;
	if (outlen < sizeof(outbuf))
...
int efx_mae_remove_ct(struct efx_nic *efx, struct efx_tc_ct_entry *conn)
{
	...
	rc = __efx_mcdi_rpc(efx, MC_CMD_MAE_FORGET_CONNECTION, inbuf, sizeof(inbuf),
			  outbuf, sizeof(outbuf), &outlen);
	if (rc)
		return rc;
	if (outlen < sizeof(outbuf))
			...
			u32 prio, u32 acts_id, u32 *id)
{
	...
	rc = __efx_mcdi_rpc(efx, MC_CMD_MAE_ACTION_RULE_INSERT, inbuf, sizeof(inbuf),
			  outbuf, sizeof(outbuf), &outlen);
	if (rc)
		return rc;
	if (outlen < sizeof(outbuf))
...
int efx_mae_update_rule(struct efx_nic *efx, u32 acts_id, u32 id)
{
	...
	MCDI_STRUCT_SET_DWORD(response, MAE_ACTION_RULE_RESPONSE_AS_ID,
			      MC_CMD_MAE_ACTION_SET_ALLOC_OUT_ACTION_SET_ID_NULL);
	return __efx_mcdi_rpc(efx, MC_CMD_MAE_ACTION_RULE_UPDATE, inbuf, sizeof(inbuf),
			    NULL, 0, NULL);
}
...
int efx_mae_delete_rule(struct efx_nic *efx, u32 id)
{
	...
	rc = __efx_mcdi_rpc(efx, MC_CMD_MAE_ACTION_RULE_DELETE, inbuf, sizeof(inbuf),
			  outbuf, sizeof(outbuf), &outlen);
	if (rc)
		return rc;
	if (outlen < sizeof(outbuf))
...
int efx_legacy_insert_rule(struct efx_nic *efx, const struct efx_legacy_match *match,
                ...
                u32 prio, u32 acts_id, u32 *id)
{
    MCDI_DECLARE_BUF(inbuf, MC_CMD_MAE_ACTION_RULE_INSERT_IN_LEN(FIREWALL_FIELD_MASK_VALUE_PAIRS_V2_LEN));
    MCDI_DECLARE_BUF(outbuf, MC_CMD_FIREWALL_ACTION_RULE_INSERT_OUT_LEN);
    MCDI_DECLARE_STRUCT_PTR(match_crit);
    //MCDI_DECLARE_STRUCT_PTR(response);
    size_t outlen;
    int rc;
    ...
    MCDI_STRUCT_SET_BYTE(match_crit, FIREWALL_FIELD_MASK_VALUE_PAIRS_V2_INGRESS_PORT,
                    match->value.ingress_port);

    MCDI_STRUCT_SET_BYTE(match_crit, FIREWALL_FIELD_MASK_VALUE_PAIRS_V2_INGRESS_PORT_MASK,
                 match->mask.ingress_port);
    /* CHAIN_INDEX */
    MCDI_STRUCT_SET_BYTE(match_crit, FIREWALL_FIELD_MASK_VALUE_PAIRS_V2_CHAIN_IDX, 0x00);

    MCDI_STRUCT_SET_BYTE(match_crit, FIREWALL_FIELD_MASK_VALUE_PAIRS_V2_CHAIN_IDX_MASK,
                 match->mask.chain_index);

    /* IP_PROTO */
    MCDI_STRUCT_SET_BYTE(match_crit, FIREWALL_FIELD_MASK_VALUE_PAIRS_V2_IP_PROTO,
                 match->value.ip_proto);
    MCDI_STRUCT_SET_BYTE(match_crit, FIREWALL_FIELD_MASK_VALUE_PAIRS_V2_IP_PROTO_MASK,
                 match->mask.ip_proto);

    /* ETH_PROTO */
    MCDI_STRUCT_SET_WORD_BE(match_crit, FIREWALL_FIELD_MASK_VALUE_PAIRS_V2_ETH_PROTO,
                    match->value.n_proto);
    MCDI_STRUCT_SET_WORD_BE(match_crit, MAE_FIELD_MASK_VALUE_PAIRS_V2_ETHER_TYPE_BE_MASK,
                    match->mask.n_proto);
    /* SRC_IP IPV6 */
    memcpy(MCDI_STRUCT_PTR(match_crit, FIREWALL_FIELD_MASK_VALUE_PAIRS_V2_SRC_IP6_BE),
                   &match->value.src_ip6, sizeof(struct in6_addr));
    memcpy(MCDI_STRUCT_PTR(match_crit, FIREWALL_FIELD_MASK_VALUE_PAIRS_V2_SRC_IP6_BE_MASK),
                   &match->mask.src_ip6, sizeof(struct in6_addr));

    /* DST_IP IPV6 */
    memcpy(MCDI_STRUCT_PTR(match_crit, FIREWALL_FIELD_MASK_VALUE_PAIRS_V2_DST_IP6_BE),
                   &match->value.dst_ip6, sizeof(struct in6_addr));
    memcpy(MCDI_STRUCT_PTR(match_crit, FIREWALL_FIELD_MASK_VALUE_PAIRS_V2_DST_IP6_BE_MASK),
                   &match->mask.dst_ip6, sizeof(struct in6_addr));

      /* SRC_IP IPV4 */
    MCDI_STRUCT_SET_DWORD_BE(match_crit, FIREWALL_FIELD_MASK_VALUE_PAIRS_V2_SRC_IP4_BE,
            match->value.src_ip);
    MCDI_STRUCT_SET_DWORD_BE(match_crit, FIREWALL_FIELD_MASK_VALUE_PAIRS_V2_SRC_IP4_BE_MASK,
            match->mask.src_ip);

    /* DST_IP IPV4 */
    MCDI_STRUCT_SET_DWORD_BE(match_crit, FIREWALL_FIELD_MASK_VALUE_PAIRS_V2_DST_IP4_BE,
            match->value.dst_ip);
    MCDI_STRUCT_SET_DWORD_BE(match_crit, FIREWALL_FIELD_MASK_VALUE_PAIRS_V2_DST_IP4_BE_MASK,
            match->mask.dst_ip);
    //printk(KERN_ERR "ip dst:%08x \n", match->value.dst_ip);

    /* SRC_PORT */
    MCDI_STRUCT_SET_WORD_BE(match_crit, FIREWALL_FIELD_MASK_VALUE_PAIRS_V2_L4_SPORT_BE,
                match->value.sport);
    MCDI_STRUCT_SET_WORD_BE(match_crit, FIREWALL_FIELD_MASK_VALUE_PAIRS_V2_L4_SPORT_BE_MASK,
                match->mask.sport);

    /* DST_PORT */
    MCDI_STRUCT_SET_WORD_BE(match_crit, FIREWALL_FIELD_MASK_VALUE_PAIRS_V2_L4_DPORT_BE,
                match->value.dport);
    MCDI_STRUCT_SET_WORD_BE(match_crit, FIREWALL_FIELD_MASK_VALUE_PAIRS_V2_L4_DPORT_BE_MASK,
                match->mask.dport);

    /* ACTION_FLAG */
    MCDI_STRUCT_SET_BYTE(match_crit, FIREWALL_FIELD_MASK_VALUE_PAIRS_V2_ACTION_FLAG,
                 act->action_flag);
    MCDI_STRUCT_SET_BYTE(match_crit, FIREWALL_FIELD_MASK_VALUE_PAIRS_V2_ACTION_FLAG0,
                 act->action_flag);
    MCDI_STRUCT_SET_BYTE(match_crit, FIREWALL_FIELD_MASK_VALUE_PAIRS_V2_ACTION_FLAG1,
                 act->action_flag);
    MCDI_STRUCT_SET_BYTE(match_crit, FIREWALL_FIELD_MASK_VALUE_PAIRS_V2_ACTION_FLAG2,
                 act->action_flag);

    rc = efx_emcdi_rpc(efx, MC_CMD_FIREWALL_RULE_ADD, inbuf, sizeof(inbuf),
              outbuf, sizeof(outbuf), &outlen, EMCDI_TYPE_FIREWALL);
    if (rc)
            return rc;
    if (outlen < sizeof(outbuf))
            return -EIO;
    idd = MCDI_DWORD(outbuf, FIREWALL_ACTION_RULE_INSERT_OUT_AR_ID);

    pr_info("Rule insertion status:%s\n",FirewallSdnetReturnTypeToString(idd));

    if (idd == FIREWALL_SDNET_CAM_ERR_DUPLICATE_FOUND)
        return -EEXIST;
    if (idd == FIREWALL_SDNET_CAM_ERR_FULL)
        return -ENOMEM;
    return -idd;
...
int delete_legacy_rule(struct efx_nic *efx, const struct efx_legacy_match *match, u32 *id)
{
    ...
    MCDI_STRUCT_SET_BYTE(match_crit, FIREWALL_FIELD_MASK_VALUE_PAIRS_V2_INGRESS_PORT_MASK,
                 match->mask.ingress_port);
    /* CHAIN_INDEX */
    MCDI_STRUCT_SET_BYTE(match_crit, FIREWALL_FIELD_MASK_VALUE_PAIRS_V2_CHAIN_IDX, 0);
                // match->value.chain_index);
    MCDI_STRUCT_SET_BYTE(match_crit, FIREWALL_FIELD_MASK_VALUE_PAIRS_V2_CHAIN_IDX_MASK,
                 match->mask.chain_index);
    ...
    rc = efx_emcdi_rpc(efx, MC_CMD_FIREWALL_RULE_DEL, inbuf, sizeof(inbuf),
              outbuf, sizeof(outbuf), &outlen, EMCDI_TYPE_FIREWALL);
...
int efx_legacy_firewall_caps(struct efx_nic *efx)
{
    ...
    rc = efx_emcdi_rpc(efx, MC_CMD_FIREWALL_CAPS, inbuf, sizeof(inbuf),
              outbuf, sizeof(outbuf), &outlen, EMCDI_TYPE_FIREWALL);
    if (rc)
            return rc;
    return 0;
