...
#endif
	if (efx_is_u25(efx) && 
		((cmd <= IPSEC_OFFLOAD_QUERY_SA) &&
		(cmd >= IPSEC_OFFLOAD_ADD_SA_ENC )))
		return efx_ipsec_ioctl(efx, ifr, cmd);


	/* IOCTL command for image upgrade */
	if ((efx_is_u25(efx)) && ((cmd >= FPGA_OPERATION) && (cmd <= FLASH_OPERATION))) {
		return image_upgrade_ioctl_call(efx, ifr, cmd);
	}
...
#if defined(EFX_USE_KCOMPAT) && defined(EFX_TC_OFFLOAD) \
    && !defined(EFX_HAVE_FLOW_INDR_BLOCK_CB_REGISTER) && !defined(EFX_HAVE_FLOW_INDR_DEV_REGISTER)
		tnl2.type = efx_tunnel_type;
		tnl2.port = (u16)ti->port;
		//pr_err("Inside udp_tnl_add_port\n");
		if (efx->type->udp_tnl_add_port2)
			efx->type->udp_tnl_add_port2(efx, tnl2);
#endif
		return;
	}
...
static void efx_udp_tunnel_del(struct net_device *dev, struct udp_tunnel_info *ti)
{
    ...
    && !defined(EFX_HAVE_FLOW_INDR_BLOCK_CB_REGISTER) && !defined(EFX_HAVE_FLOW_INDR_DEV_REGISTER)
		...
		if (efx->type->udp_tnl_del_port2)
			efx->type->udp_tnl_del_port2(efx, tnl2);
#endif
		return;
	}
...
int efx_netevent_event(struct notifier_block *this, unsigned long event,
		void *ptr)
{
...
static int efx_register_netdev(struct efx_nic *efx)
{
	...
	efx_devlink_sriov_configure(efx);
...
#if 1
	fpga.nvram_id = 0x0; /* Primary flash */
	if (select_fpga_nvram(efx, &fpga) != 0)
		netif_info(efx, drv, efx->net_dev, "FPGA flash selected: Primary\n");

	if (efx_is_u25(efx) && (reset)) {
		if (reset_fpga(efx) == 0) {
			 ...
			 const struct pci_device_id *entry)
{
	...
	*probe_ptr = probe_data;
	...
	if (efx_is_u25(efx)) {
		efx->netevent_notifier.notifier_call = efx_netevent_event;
		...
		int num_vfs)
{
	...
	if (nic_data->mode == U25_MODE_SWITCHDEV && efx_is_u25(efx)) {
		netif_info(efx, drv, efx->net_dev, "Device not supported vf creation in Switchdev mode\n");
		return -EOPNOTSUPP;
	} 
...
static netdev_tx_t __efx_hard_start_xmit(struct sk_buff *skb,
		struct net_device *net_dev)
{
	struct efx_nic *efx = efx_netdev_priv(net_dev);
	...
	if (!efx_is_u25(efx))
		return efx_hard_start_xmit(skb, net_dev);
	...
	nic_data = efx->nic_data;
	outer_vlan_id = U25_MPORT_TO_VLAN(nic_data->pf_mport);
	inner_vlan_id = U25_MPORT_TO_VLAN(nic_data->base_mport);

	__skb_push(skb, offset);
	__vlan_insert_tag(skb, htons(ETH_P_8021Q), inner_vlan_id);
	__skb_pull(skb, offset);

	__skb_push(skb, offset);
	__vlan_insert_tag(skb, htons(ETH_P_8021Q), outer_vlan_id);
	__skb_pull(skb, offset);

	skb->protocol = htons(ETH_P_8021Q);
	//pkt_hex_dump(skb->data, net_dev->name);
...
const struct net_device_ops efx_netdev_ops = {
	...
	.ndo_start_xmit		= __efx_hard_start_xmit,
