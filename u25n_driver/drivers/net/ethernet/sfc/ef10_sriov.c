...
int efx_ef10_sriov_set_vf_mac(struct efx_nic *efx, int vf_i, u8 *mac)
{
	...
	if (vf->efx) {
		...
		if (rc == 0) {
			...
			if (nic_data->is_u25 && (nic_data->mode == U25_MODE_SWITCHDEV)) {
				struct efx_vfrep *efv = netdev_priv(nic_data->vf_rep[vf_i]);
				ether_addr_copy(efv->net_dev->dev_addr, mac);
			}
			return 0;
	...
	if (!is_zero_ether_addr(vf->mac)) {
		...
		if (nic_data->is_u25 && (nic_data->mode == U25_MODE_SWITCHDEV)) {
			struct efx_vfrep *efv = netdev_priv(nic_data->vf_rep[vf_i]);
			eth_zero_addr(efv->net_dev->dev_addr);
			printk("****DGB**** in %s %d\n", __func__,  __LINE__);
		}
	}
